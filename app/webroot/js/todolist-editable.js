$(function() {
    $( ".sortable" ).sortable({
        ...
        start: function(event, ui) {
            ...
            $('.sortablePlaceholder').css('height',ui.item.outerHeight());
        },
        ...
        update: function(event, ui) {
            ...
            $('.column').each(function(index) {
                $(this).find('.numberOfTodos').html($(this).find('li.todo').length);
            });
    ...
    $('.edit_button').click(function() {
        editElement($(this).parent().parent().parent().parent());
    });
...
function updateULHeight(){
    maxHeight = 0;
    $('.column.ul').each(function(index,ul) {
        height = $(ul).height();
...
function resizeActionsBlocks(){
    $('.actions').each(function(index) {
        $(this).height($(this).parent().innerHeight()-10);
    });
}