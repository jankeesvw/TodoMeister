...
class TodosController extends AppController {
    ...
    function start($project_id = "") {
        ...
        if (isset($_POST['projectname']) && isset($_POST['username']) && $_POST['projectname'] !== "" && $_POST['username'] !== "") {
            $this->redirect(array('controller' => 'todos', 'action' => 'todolist', 'project_id' => $_POST['projectname'], 'name' => $_POST['username']));
            exit;
        }
    ...
    function todolist($project_id, $name = "") {
        ...
        if ($auth_level > 1) {
            $locklink = $html->link($html->image($html->url('/img/logbook.png'), array('title' => 'open project log')), array('action' => 'project_log', $project_id, $name), array('escape' => false));
    ...
    function login($project_id, $name = "") {
        if ($this->data) {
            if ($this->Authorization->doLogin($project_id, $this->data['Password']['password']) > 0) {
                $this->redirect(array('controller' => 'todos', 'action' => 'todolist', 'project_id' => $project_id, 'name' => $name));
                exit;
            } else {
    ...
    function project_log($project_id, $name) {
        ...
        App::import('Helper', 'Html');
        $html = new HtmlHelper();
        $this->set('custom_title', 'Project log for: <a href="' . $html->url(array('controller' => 'todos', 'action' => 'todolist', 'project_id' => $project_id, 'name' => $name)) . '">' . $project_id . '</a>');
    ...
    function add($project_id, $name) {
        ...
        // redirect back to the list with a #status to regain focus
        $this->redirect(array('controller' => 'todos', 'action' => 'todolist', 'project_id' => $project_id, 'name' => $name, '#' => trim($this->data['Todo']['status'])));
        exit;
    }
    ...
    function log_item($id, $project_id, $name) {
        ...
        App::import('Helper', 'Html');
        $html = new HtmlHelper();
        $this->set('custom_title', '<a href="' . $html->url(array('controller' => 'todos', 'action' => 'todolist', 'project_id' => $project_id, 'name' => $name)) . '">Item log</a>');
    }
    ...
    function updateStatus($id, $new_status, $project_id, $name) {
        if ($this->Authorization->checkAuthorization($project_id) < 2) {
            // redirect back to the login page
            $this->Session->setFlash('This method requires a login');
            $this->redirect(array('controller' => 'todos', 'action' => 'login', 'project_id' => $project_id, 'name' => $name));
            exit;
        }
